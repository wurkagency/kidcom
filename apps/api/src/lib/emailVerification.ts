...
export async function sendVerificationEmail(user: { id: string; email: string; firstName: string }) {
  ...
  await mailSender.send({
    to: user.email,
    subject: "Verify your email for Kinnd",
    text: `Hi ${user.firstName},\n\nPlease confirm this is your email address to finish setting up your Kinnd account:\n\n${link}\n\nThis link expires in ${VERIFICATION_TOKEN_TTL_HOURS} hours. If you didn't create a Kinnd account, you can ignore this email.`,

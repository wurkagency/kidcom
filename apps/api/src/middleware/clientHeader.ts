...
// CSRF defence on top of the SameSite=Lax session cookie. The Kinnd app
...
// form post or <img> from anywhere else — including another *.kinnd.eu
// site, which SameSite treats as "same site" — can't change anything.
export const CLIENT_HEADER = "X-Kinnd-Client";
...
export function requireClientHeader(req: Request, _res: Response, next: NextFunction) {
  ...
  if (req.get(CLIENT_HEADER) !== "1") {
    return next(new ApiError(403, "This request must come from the Kinnd app", "CLIENT_HEADER_REQUIRED"));
  }
  next();
}
